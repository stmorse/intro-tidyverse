...
# Let's over-write the `price` column with the numeric version of price in it:	
...
# Well, the first thing we need to do is make sure the prices is in a numeric form.  We already dealt with this before by overwriting a new column using the dollar-sign base R syntax.  Let's instead take a tidy R approach and `mutate` the listings data table by adding the changes right in our chain:  	
listings %>%	
  mutate(weekly = as.numeric(gsub('\\$|,', '', weekly_price)),	
         weekly_price_per = weekly / 7,	
         weekly_discount = price - weekly_price_per) %>%	
  summarize(avg_discount = mean(weekly_discount, na.rm=T))

# This approach has several advantages over the base R way.  One advantage is we can use the column temporarily, as part of our chain, without affecting the data table that we have loaded into memory.  Another advantage is we can create a new column, and then use those new values immediately in another column! 

# Average discount per day for booking by the week: about 20 bucks! 

# Note: we used the argument `na.rm=TRUE` in `mean`, which just removes any NA values from the mean computation --- we could have also chained another filter command with the same result.	
...
# That scatterplot of the price against the review score seemed interesting, we'd like to revisit it.  (We'll be using the `price` variable, make sure it's in numeric format, e.g. by running a `str(listings$price)` command.)	
...
# That looks pretty good if you ask me!  And, like any decent visualization, it tells a story, and raises interesting questions: there appears to be a correlation between the score of a listing and the price --- is this because higher rated listings can ask a higher price on average? or because when you pay top dollar you trick yourself into believing it was a nicer stay?  What is the best predictor of price, or ratings, or other variables?    	

# Finally, let's fit a trend line to that scatter plot of average price vs review score by adding a `geom_smooth` call.	 We'll specify we want a linear regression (default is a "loess" fit, locally weighted scatterplot smoothing)..
listings %>%	
  filter(!is.na(review_scores_rating)) %>%	
  group_by(review_scores_rating) %>%	
  summarize(med.price = median(price),	
...
# Note that all we did was take our previous plot commands, and added the `geom_smooth` command.  This is the *stat*istical transformation we mentioned.	
...
# There is also `stat_smooth` and others, and endless ways to specify the complexity, paramaterization, and visualization properties of the model.
...
# Before we do, would you mind saving a "color-blind" palette for us to use?  I'm not crazy about the default color scheme (or palette) which is green and red, and I am red-green colorblind. Woe is me and my fellow colorblind peoples.
cbbPalette = c("#E69F00", "#56B4E9", "#009E73", 
               "#F0E442", "#0072B2", "#D55E00", "#CC79A7")

# Now back to using long/wide data.  With the long format data, we can simply specify the color of our line with the `freq` column, which gives which type of observation it is.	
long.price %>%	
  filter(tprice < 1000) %>%	
...
listings %>%	
  ...
  gather(key=type, value=number, Bedrooms, Bathrooms) %>%	
